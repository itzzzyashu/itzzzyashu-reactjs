...
export default class BlogItem extends Component {
    render() {
        const { content_pic, content_url, content_title, content_description, content_posted_on } = this.props;
        ...
        let content_posted_time = `${content_posted_day} ${content_posted_date}-${content_posted_month}-${content_posted_year} / ${content_posted_hour}:${content_posted_minute} ${am_pm}`;
        // let content_title_no_hyphen_in_lowecase = content_title.replace(/\s+/g, '-').toLowerCase();

        // let content_url = `${content_posted_year}/${content_posted_month}/${content_posted_date}/${content_title_in_hyphen_in_lowecase}`;

        return (
            <>
                <div className="h-[24rem] flex flex-col justify-between sm:w-1/2 md:w-1/3 lg:w-1/4 pb-1 p-4 my-4 overflow-hidden bg-transparent transition">
                    <img className='w-full h-52 object-cover rounded-xl hover:object-center duration-200' src={content_pic} alt="Blog pic" />
                    <div className="items-start p-1">
                        <a target="_blank" rel='noreferrer' href={content_url}>
                        ...
                        <p className='text-gray-400 text-xs mt-1 mb-1 selection:bg-gray-200 dark:selection:bg-gray-900'>
                            Updated on <span className="post-date-font">{content_posted_time}</span>
                        </p>
                        ...
                        <a target="_blank" rel='noreferrer' href={content_url} className='py-1 inline-flex w-fit cursor-pointer text-sm bg-transparent font-bold hover:ml-3 duration-200'> ⪼ View </a>
                    </div>
                </div>
            </>
        )
